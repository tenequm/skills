...
metadata:
  ...
  openclaw:
    ...
    emoji: "📅"
    primaryEnv: GLIM_API_KEY
    requires:
      env:
        - GLIM_API_KEY
      config:
        - "~/.config/last30days-glim/.env"
        - ".claude/last30days-glim.env"
    envVars:
      - name: GLIM_API_KEY
        required: true
        description: Glim MCP API key. Required for full multi-platform research; partial baseline works without it.
      - name: XAI_API_KEY
        required: false
        description: Optional xAI key for direct LLM judging (otherwise routed via glim).
      - name: OPENAI_API_KEY
        required: false
...
- **Reddit**: direct `reddit.com/.json` (free baseline, gets comments). On rate-limit / anti-bot, falls back to `glim_reddit_search`.
- **X / Twitter**: `glim_twitter_search` with `since:` filter at today-30. Hard cap 2 fetches per run.
- **YouTube**: `glim_web_search site:youtube.com` for discovery + `glim_youtube_subtitles` for transcripts.
- **GitHub**: direct `api.github.com` (anonymous, 60/hr). On rate-limit, falls back to `glim_github_search`.
- **Hacker News**: direct `hn.algolia.com/api/v1` (free, reliable). On exception, glim web/crawl json fallback.
- **Polymarket**: direct `gamma-api.polymarket.com` (free, reliable). On exception, glim web/crawl json fallback.
- **Bluesky**: glim web/crawl json against AT Protocol (direct egress gets 403).
- **TikTok / Instagram / Threads / Pinterest**: glim web/crawl. Sources cleanly mark unavailable on bot challenge / login wall — they are NOT fabricated.
- **Web baseline (always-on)**: `glim_web_search(query, published_within_days: 30)`.

After fan-out: normalize, dedupe (Jaccard 0.7 within-source, URL-key across sources), RRF fusion, per-author cap (max 3), LLM-judge for relevance + fun (via glim inference at `/api/v2/inference/v1/chat/completions`, model `google/gemini-3.1-flash-lite-preview`), cluster (when intent ∈ {breaking_news, opinion, comparison, prediction}), render with the EVIDENCE / PASS-THROUGH FOOTER envelopes.
...
```
🌐 last30days-glim v0.1.0 · synced {YYYY-MM-DD}
...
# {A} vs {B} [vs {C}]: What the Community Says (/Last30Days-Glim)
...
> 🔍 Research Coverage: {N}/{M} sources active. Light coverage on {missing}. Two reasons that happens: (1) some platforms (TikTok/Instagram/Threads/Pinterest) need glim's mobile-proxy escalation and may be temporarily unavailable; (2) the topic may be too niche for some platforms.

## Failure / refuse cases

- **Glim unavailable** (`GLIM_API_KEY` unset): degraded run with the four free-baseline sources only. Tell the user.
...
- [setup.md](references/setup.md) — glim install, OAuth, balance top-up, troubleshooting
