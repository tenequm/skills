...
## Default models (glim inference, OpenAI-compat)
...
Override via env: `LAST30DAYS_PLANNER_MODEL`, `LAST30DAYS_RERANK_MODEL`. Any model in glim's inference router enum works (Anthropic Claude 4.5/4.6, xAI Grok 4.1/4.20, MiniMax, GLM, Qwen, Kimi, etc.) — provider prefix required.

## Endpoint

```
POST https://surf.cascade.fyi/api/v2/inference/v1/chat/completions
Authorization: Bearer $GLIM_API_KEY
