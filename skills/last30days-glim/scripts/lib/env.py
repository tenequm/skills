"""Environment + config loading for last30days-glim.

Single auth source: GLIM_API_KEY. All upstream multi-provider / cookie /
Codex-CLI machinery is removed because glim replaces it.
...
The skill's adapters check `glim_client.is_available()` at call time;
provider routing is hard-pinned to "glim" via LAST30DAYS_REASONING_PROVIDER
...
if _config_override == "":
    ...
    CONFIG_DIR = Path.home() / ".config" / "last30days-glim"
    CONFIG_FILE = CONFIG_DIR / ".env"

PROJECT_ENV_FILENAME = "last30days-glim.env"
...
def _check_file_permissions(path: Path) -> None:
    try:
        ...
        if mode & 0o044:
            sys.stderr.write(
                f"[last30days-glim] WARNING: {path} is readable by other users. "
                f"Run: chmod 600 {path}\n"
            )
            sys.stderr.flush()
    except OSError as exc:
        sys.stderr.write(f"[last30days-glim] WARNING: could not stat {path}: {exc}\n")
        sys.stderr.flush()
...
def _find_project_env() -> Path | None:
    """Walk up from cwd looking for .claude/last30days-glim.env."""
    cwd = Path.cwd()
...
def get_config() -> dict[str, Any]:
    ...
    glim_api_key = os.environ.get("GLIM_API_KEY") or merged_env.get("GLIM_API_KEY")

    config: dict[str, Any] = {
        # Glim is the canonical (and only) auth path.
        "GLIM_API_KEY": glim_api_key,
        # Provider hard-pinned to glim so upstream's auto-resolution doesn't
        # try Gemini/OpenAI/xAI/OpenRouter chains. The vendored providers.py
        # exposes a single GlimRouter that handles this.
...
def get_x_source(config: dict[str, Any]) -> str | None:
    ...
    For our port, X is served via glim when GLIM_API_KEY is set.
    Returns "glim" or None.
    """
    return "glim" if config.get("GLIM_API_KEY") else None


def get_x_source_with_method(config: dict[str, Any]) -> tuple[str | None, str]:
    return (get_x_source(config), "glim")
...
# now either glim-routed or out-of-scope. These return glim-availability so
# pipeline.py's availability checks light up the right sources.
# ---------------------------------------------------------------------------

def _glim_available(config: dict[str, Any]) -> bool:
    return bool(config.get("GLIM_API_KEY"))


def is_youtube_sc_available(config: dict[str, Any]) -> bool:
    return _glim_available(config)
...
def is_truthsocial_available(config: dict[str, Any]) -> bool:
    return False  # out of scope: glim has no Truth Social path


def is_xiaohongshu_available(config: dict[str, Any]) -> bool:
    return False  # out of scope: glim has no Xiaohongshu path


def is_xquik_available(config: dict[str, Any]) -> bool:
    return False  # x-cluster collapsed to single glim twitter primitive
...
def get_x_source_status(config: dict[str, Any]) -> dict[str, Any]:
    """Compat shim — diagnose() reads this. We collapse to a single glim-based
    status because the four upstream X backends are gone."""
    return {
        "source": "glim" if _glim_available(config) else None,
...
def get_openai_auth(file_env: dict[str, str]):
    ...
    minimal duck-typed namespace because glim replaces OpenAI entirely."""
    class _Auth:
        token: str | None = None
