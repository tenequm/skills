...
Uses glim web search (single backend) to discover relevant communities and context
...
from . import glim_adapters as grounding  # glim v2 web/search replaces the multi-backend grounding module
...
def _has_backend(config: dict) -> bool:
    """Check if a web search backend is available.

    last30days-glim: glim is the single backend, gated by GLIM_API_KEY.
    """
