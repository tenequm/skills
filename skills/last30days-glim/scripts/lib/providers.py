"""Reasoning provider routing for last30days-glim.

Single provider: glim inference (OpenAI-compatible chat completions endpoint
on the glim API, billed against the embedded balance via GLIM_API_KEY).
...
When GLIM_API_KEY is unset, resolve_runtime() returns the upstream "local"
...
from . import env, schema, glim_client

# Default models. Both pins must include the glim-required `google/` prefix.
...
# these endpoints are actually called in our port — glim inference replaces
# them all.
GEMINI_URL = ""
...
class GlimReasoningClient(ReasoningClient):
    """Routes planner + rerank LLM calls through glim's OpenAI-compatible
    inference endpoint. Single auth path: GLIM_API_KEY."""
    ...
    def generate_text(
        ...
        *,
        ...
        try:
            payload = glim_client.post_inference(
                model=model,
                messages=messages,
                temperature=0,
                response_format=response_format,
            )
        except glim_client.GlimUnavailable:
            return ""
        except glim_client.GlimError as e:
            print(
                f"[Providers] glim inference {e.status_code}: "
                f"{(e.body or '')[:300]}",
                file=sys.stderr,
            )
            return ""
...
def _resolve_x_backend(config: dict[str, Any]) -> str | None:
    """X/Twitter backend selector. Glim is the only path in our port."""
...
def mock_runtime(config: dict[str, Any], depth: str) -> schema.ProviderRuntime:
    ...
    provider_name = (config.get("LAST30DAYS_REASONING_PROVIDER") or "glim").lower()
...
def resolve_runtime(
    ...
    If GLIM_API_KEY is configured (and provider is "glim"/"auto"), returns a
    GlimReasoningClient that routes LLM calls through glim inference.
    ...
    glim_key = config.get("GLIM_API_KEY")

    if provider_name in ("auto", "glim") and glim_key:
        planner_model, rerank_model = _resolve_model_pins(config, depth, "glim")
        ...
        return runtime, GlimReasoningClient()
...
def extract_gemini_text(payload: dict[str, Any]) -> str:
    """Compat shim. Glim does not return Gemini-native shapes; this exists
