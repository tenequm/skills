"""Cluster-first rendering for the last30days-glim pipeline."""
...
def _render_badge() -> list[str]:
    ...
    return [
        f"🌐 last30days-glim v{version} · synced {today}",
        "",
    ]
...
def render_compact(report: schema.Report, cluster_limit: int = 8, fun_level: str = "medium", save_path: str | None = None) -> str:
    ...
    lines = [
        *_render_badge(),
        f"# last30days-glim v{_skill_version()}: {report.topic}",
        "",
...
def _render_canonical_boundary() -> list[str]:
    ...
    return [
        "",
        "---",
        "# END OF last30days-glim CANONICAL OUTPUT",
        "",
...
def render_comparison_multi(
    ...
    *,
    ...
    lines: list[str] = [
        *_render_badge(),
        f"# last30days-glim v{_skill_version()}: {synthesized_topic}",
        "",
        *_assistant_safety_lines(),
