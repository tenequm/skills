...
def ensure_supported_python(version_info: tuple[int, int, int] | object | None = None) -> None:
    ...
    if (major, minor) >= MIN_PYTHON:
        return
    sys.stderr.write(
        "last30days-glim requires Python 3.12+.\n"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--web-backend", default="auto",
                        choices=["auto", "glim", "none"],
                        help="Web search backend (default: auto, routes through glim)")
...
def _show_runtime_ui(
    ...
    # The `web` promo nudges users to set GLIM_API_KEY, which
...
def main() -> int:
    ...
    if topic.lower() == "setup":
        sys.stderr.write(
            "last30days-glim has no interactive setup wizard.\n"
            "Set GLIM_API_KEY (env var, .claude/last30days-glim.env, or "
            "~/.config/last30days-glim/.env) and you are done.\n"
            ...
            if comp_explicit:
                ...
                if not resolve_mod._has_backend(config) and not args.mock:
                    sys.stderr.write(
                        ...
                        "  3. Re-invoke: /last30days-glim '{topic} vs {peer1} vs {peer2}' "
