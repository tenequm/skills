...
def ensure_supported_python(version_info: tuple[int, int, int] | object | None = None) -> None:
    ...
    if (major, minor) >= MIN_PYTHON:
        return
    sys.stderr.write(
        "last30days-surf requires Python 3.12+.\n"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--web-backend", default="auto",
                        choices=["auto", "surf", "none"],
                        help="Web search backend (default: auto, routes through surf)")
    ...
    parser.add_argument("--github-repo", help="Comma-separated owner/repo for project-mode search (e.g., anthropics/claude-code,microsoft/vscode)")
    parser.add_argument(
        "--competitors",
        nargs="?",
        const=2,
...
def _show_runtime_ui(
    ...
    # The `web` promo nudges users to set SURF_API_KEY, which
...
def main() -> int:
    ...
    # subcommand flags pass through without argparse hard-exiting.
    ...
    if topic.lower() == "setup":
        sys.stderr.write(
            "last30days-surf has no interactive setup wizard.\n"
            "Set SURF_API_KEY (env var, .claude/last30days-surf.env, or "
            "~/.config/last30days-surf/.env) and you are done.\n"
            "See references/setup.md for funding the embedded balance.\n"
        )
        return 0
            ...
            if comp_explicit:
                ...
                if not resolve_mod._has_backend(config) and not args.mock:
                    sys.stderr.write(
                        ...
                        "  3. Re-invoke: /last30days-surf '{topic} vs {peer1} vs {peer2}' "
