...
def _fallback_plan(
    ...
    # the literal phrase. Fixes the historical literal-phrase failure.
...
def _infer_intent(topic: str) -> str:
    ...
    if re.search(r"\b(trending|this week|right now|today|this month)\b", text):
        return "breaking_news"
    # Default is "concept" rather than "breaking_news":
    # the literal-phrase failure: unclassified topics were getting
...
def _keyword_query(topic: str, core: str) -> str:
    ...
    Quote ONLY title-cased multi-word proper nouns ("Topic Phrase",
    ...
    topic, producing searches like `"Topic Phrase" topic actual`
...
def _intent_modifier_subqueries(
    ...
    (e.g., "topic with intent modifier") into every search_query. This helper
...
def _max_subqueries(intent: str, topic: str | None = None) -> int:
    ...
    # (use cases, workflows, examples, review, etc.). The historical
    # literal-phrase failure: prior cap of 3 produced near-literal
    ...
    # the intent itself is factual/concept. Without this, a topic-phrase query (classified "concept" by default
