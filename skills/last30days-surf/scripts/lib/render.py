"""Cluster-first rendering for the last30days-surf pipeline."""
...
def _skill_version() -> str:
    """Read version from the skill's SKILL.md frontmatter.

    Walks up from render.py's location to find the skill root, then parses
    the metadata.version field from SKILL.md. Falls back to "?" on any error
    so the badge emission never crashes the run.
    """
...
def _render_badge() -> list[str]:
    ...
    Added in v3.0.8 after three self-debugs confirmed
    ...
    return [
        f"🌐 last30days-surf v{version} · synced {today}",
        "",
    ]
...
def render_compact(report: schema.Report, cluster_limit: int = 8, fun_level: str = "medium", save_path: str | None = None) -> str:
    ...
    lines = [
        *_render_badge(),
        f"# last30days-surf v{_skill_version()}: {report.topic}",
        "",
...
def _render_canonical_boundary() -> list[str]:
    ...
    Added in v3.0.9 after an earlier self-debug
    ...
    Updated after the literal-phrase failure: the prior
    ...
    return [
        "",
        "---",
        "# END OF last30days-surf CANONICAL OUTPUT",
        "",
        ...
        "of synthesizing - STOP and regenerate. This is the historical literal-evidence-dump",
...
def _render_degraded_run_warning(report: schema.Report) -> list[str]:
    ...
    LAW 7. The an earlier failure mode Run 1 failure mode: the
...
def render_comparison_multi(
    ...
    *,
    ...
    lines: list[str] = [
        *_render_badge(),
        f"# last30days-surf v{_skill_version()}: {synthesized_topic}",
        "",
        *_assistant_safety_lines(),
