...
# marginal matches
# an unrelated video scored 51 / ranked #2 with zero
# Hermes content.
ENTITY_MISS_PENALTY = 25.0

# Intent modifiers to strip before extracting the primary entity so that,
# for example, "Topic Phrase" yields primary_entity="topic"
# rather than "topic with intent modifier". Kept in sync with
...
def _build_prompt(topic: str, plan: schema.QueryPlan, candidates: list[schema.Candidate], primary_entity: str = "") -> str:
    ...
    if primary_entity:
        grounding_hint = (
            ...
            "entity itself. the historical literal-phrase failure: an unrelated video "
...
def _fallback_tuple(candidate: schema.Candidate, *, primary_entity: str = "") -> tuple[float, str]:
    ...
    # thin-text sources unfairly. An unrelated
...
#: the composite signal the cluster-scoring layer consumes.
